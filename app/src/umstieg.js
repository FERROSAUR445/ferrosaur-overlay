// Umstieg auf das neue Ferrosaur HUD (Overlay 2.0) — die LETZTE Version des alten Overlays.
//
...
//   2. HUD-Installer vom Backend laden (<TOKEN_BASE>/overlay/Ferrosaur-HUD-Setup.exe —
...
const HUD_SETUP_NAME = 'Ferrosaur-HUD-Setup.exe';
const HUD_TEST_SETUP_NAME = 'Ferrosaur-HUD-Test-Setup.exe';
...
<h1>🦖 Umstieg auf das neue Ferrosaur HUD</h1>
...
function download(url, dest, onProgress) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'User-Agent': 'Ferrosaur-Overlay-Umstieg' } }, (res) => {
...
function hudDir(test) {
  ...
  return path.join(base, test ? 'Ferrosaur HUD (Test)' : 'Ferrosaur HUD');
}
...
function hudExe(test) {
  ...
  try {
    ...
    const bevorzugt = kandidaten.find((f) => /ferrosaur/i.test(f));
...
async function umstiegAusfuehren(tokenBase) {
  ...
  const win = new BrowserWindow({
    ...
    title: 'Ferrosaur HUD 2.0', autoHideMenuBar: true, alwaysOnTop: true,
  ...
  await win.loadURL('data:text/html;charset=utf-8,' + encodeURIComponent(html('Ferrosaur HUD 2.0')));
  ...
  try {
    ...
    const link = 'https://ferrosaur.de/profil';
