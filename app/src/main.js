...
// app/scripts/patch-prod.js auf 'Ferrosaur Overlay' zurück). Electron leitet
...
app.setName('Ferrosaur Overlay Test');

// Einmal-Migration des userData-Ordners — NUR im Prod-Build (patch-prod setzt den Namen auf
// 'Ferrosaur Overlay'). Grund: erst seit dem app.setName()-Aufruf oben ist der Ordner
// deterministisch der productName. Hieß der bisher installierte Prod-Build seinen Ordner anders
// (z. B. 'ferrosaur-overlay' aus package.json name, wenn electron-builder den productName NICHT
...
if (app.getName() === 'Ferrosaur Overlay') {
  try {
    const cur = app.getPath('userData');
    if (!fs.existsSync(path.join(cur, 'session.json'))) {
      const alt = path.join(path.dirname(cur), 'ferrosaur-overlay');
...
const TOKEN_BASE = 'https://api-test.ferrosaur.de';
// Discord-Server (Guild) von Ferrosaur — die eigentliche "bist du auf unserem Server"-Prüfung
// läuft im Backend (ferrosaur-backend), das nicht Teil dieses Repos ist. Diese ID hier ist nur
// eine Referenz-Konstante, falls sie clientseitig (z. B. Einladungs-Link, Anzeige) gebraucht wird.
const DISCORD_GUILD_ID = '1531054994533646536';
...
// patch-prod.js dreht das für den Prod-Release auf 'ferrosaur' zurück (analog zu appId/URL).
// Der echte Login läuft über diesen Scheme: das Backend redirectet nach dem OAuth auf
// APP_REDIRECT (Default ferrosaur://auth) → das OS öffnet die App, die den Scheme registriert
// hat. Ohne eigenen Scheme öffnete ein Test-Login ggf. die Prod-App mit einer test-signierten
// Session, die api.ferrosaur.de ablehnt. Dazu MUSS das api-test-Backend
// APP_REDIRECT=ferrosaur-test://auth setzen (internal/config/config.go).
const SCHEME = 'ferrosaur-test';
...
function createTray() {
  ...
  tray.setToolTip('Ferrosaur Overlay Test');
...
function createLoginWindow() {
  loginWindow = new BrowserWindow({
    width: 440, height: 580, resizable: false, title: 'Ferrosaur Login Test',
