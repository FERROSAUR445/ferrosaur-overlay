...
async function init() {
  ...
  el('updateBtn').onclick = () => {
    ...
    else if (updateState === 'error') { window.bf.openExternal?.(`${config.tokenBase}/overlay/`); }   // manueller Download-Fallback über das Backend (kein öffentliches GitHub-Repo)
  };
...
function renderTpList() {
  ...
  for (const t of [...teleports].sort((a, b) => a.number - b.number)) {
    ...
    const water = !!t.water;
    const row = document.createElement('div');
    row.style.cssText = `padding:8px 10px;margin-bottom:8px;border-radius:8px;cursor:pointer;border:1.5px solid ${hot ? 'var(--accent)' : (water ? 'rgba(56,189,248,0.7)' : 'rgba(255,255,255,0.35)')};background:${hot ? 'rgba(var(--accent-rgb),0.20)' : (water ? 'rgba(56,189,248,0.16)' : 'rgba(20,16,28,0.65)')};box-shadow:0 2px 5px rgba(0,0,0,0.5)`;
    row.innerHTML =
...
function closeAllFeatures(skipInteractive) {
  ...
  ['dinoInfo', 'skinEditor', 'garage', 'market', 'group', 'profile', 'lexikon', 'leaderboard', 'lootbox', 'notifications'].forEach((id) => { el(id).style.display = 'none'; });
...
async function loadSoftwareTab() {
  ...
  try {
    ...
    // Release-Notes kommen ausschließlich vom eigenen Backend (kein öffentliches GitHub-Repo vorhanden).
...
// HUD-Pille (oben mittig) & Große Karte sind bewusst NICHT enthalten = nicht verschiebbar.
